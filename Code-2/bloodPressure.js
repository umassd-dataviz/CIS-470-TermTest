...
function BloodPressure(patientName, patientAge,systolic,diastolic) {
  ...
  const severityLevels = {
    ...
    "HYPERTENSIVE_CRISIS": 4
      ...
      if (systolic < 120) { // ERROR needs to be less than not greater
        systolicDiagnosis = "NORMAL";
      } else if (systolic < 140) {
        systolicDiagnosis = "PREHYPERTENSION";
      } else if (systolic < 160) {
        systolicDiagnosis = "STAGE_1_HBP";
      } else if (systolic <= 180) { // ERROR should be less than or equal to 180 to prevent preemting
      ...
      } else if (diastolic <= 110) { // ERROR should be less than or equal to 110 to prevent preemption
