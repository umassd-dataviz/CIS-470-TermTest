function calculatePrice(price, isStudent, hasCoupon) {
    ...
    // Check if final price is greater than input price
    if (finalPrice > price) {
      throw new Error("Invalid price: Output price must be less than input price.");
    }
    /* if (finalPrice < 0.8 * price) {
      throw new Error("Invalid price: Price must be a positive number.");
    }
    */ 
    return finalPrice;
}
module.exports = calculatePrice; // Changed export
